...
try:
    ...
    raise ImportError(
        "缺少依赖 deap。请先安装：pip install deap"
    ) from exc
...
def load_peak_fno_checkpoint(ckpt_path, device="cpu"):
    ...
    required_keys = {"MODES", "WIDTH", "DEPTH", "LAM_FF", "HEAD_HIDDEN"}
    missing_cfg = sorted(required_keys.difference(cfg.keys()))
    if missing_cfg:
        raise RuntimeError(
            "这个优化脚本当前接的是旧的 2D S 参数 FNO checkpoint，"
            f"但你给的 checkpoint 缺少这些字段: {missing_cfg}。"
            "如果你想直接优化当前 full-field dual 模型，需要再做一版 3D dual surrogate 优化器。"
        )
...
def prepare_optimizer_config(cfg):
    ensure_standard_dirs()

    model_cfg = dict(cfg.get("model", {}))
    choice = str(model_cfg.get("choice", "path" if cfg.get("ckpt_path") else "current_best"))
    current_best_name = str(model_cfg.get("current_best_name", "fno_peak_curve_best.pt"))
    current_final_name = str(model_cfg.get("current_final_name", "fno_peak_curve_final.pt"))

    resolved_ckpt_path, checkpoint_source = resolve_checkpoint_choice(
        choice,
        current_best=MODELS_CURRENT_DIR / current_best_name,
        current_final=MODELS_CURRENT_DIR / current_final_name,
        history_root=BEST_MODEL_HISTORY_ROOT,
        custom_path=model_cfg.get("custom_path", cfg.get("ckpt_path")),
    ...
    output_root = Path(cfg.get("output_dir_root", OPTIMIZATION_OUTPUTS_DIR))
...
def run_nsga2(cfg):
    ...
    device = cfg["device"]
    model, lambda_vec, meta = load_peak_fno_checkpoint(cfg["resolved_ckpt_path"], device=device)

    print("[Model] loaded.")
    print("[Model] checkpoint =", cfg["resolved_ckpt_path"])
    print("[Model] source =", cfg.get("checkpoint_source"))
    ...
    with open(Path(run_dir) / "config_resolved.json", "w", encoding="utf-8") as f:
...
def main():
    ...
    cfg = prepare_optimizer_config(cfg)
