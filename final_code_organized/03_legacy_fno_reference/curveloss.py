...
CFG = {
    ...
    "train": {
        "epochs": 3000,             #最大训练轮次
        "batch_size": 32,          # 曲线段训练显存更吃紧；可用64/32
        "val_batch_size": 64,
        "k_lam_per_sample":90,    # 每结构抽K个λ点
        "n_use": 5000,             # 只用前N条有效样本；None=全部
        "val_ratio": 0.2,
        "eps_valid": 1e-12,
        "seed": 0,
        "plot_every": 10,
    ...
    "sampling": {
        ...
        "mix_uniform_ratio": 0.4,     # 0~1；0=不混合(保持你当前逻辑)；例如0.5表示一半均匀一半偏峰
    },
    ...
    "early_stop": {
        ...
        "min_delta": 1e-9,    # 认为“有改善”的最小下降幅度
        "warmup": 50,         # 前多少epoch不做early stop（避免一开始抖动误判）
