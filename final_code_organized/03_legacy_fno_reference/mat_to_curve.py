# ==========================================================
# 曲线段训练版（S参数复数监督）——2D Fourier + DWConv(方案2) + 曲线段级 STFT 损失
# - 训练逻辑保持不变（Dataset/采样/训练循环/输出接口不变）
# - 模型：Fourier2DEncoder(2D FNO) + DWConv3×3+1×1 (Depthwise Separable) + λ Fourier Features + MLP head
# - 损失：点级 MSE(复数S) +（可选）曲线段级 STFT loss（对 A(λ) 的局部时频一致性）
# ==========================================================
...
CFG = {
    "paths": {
        ...
        "save_path":     r"C:\Users\90740\Desktop\final\fourier2d_dwconv_stft_curve.pt",
        "log_root":      r"runs/fourier2d_dwconv_stft_curve",
    },
    ...
    "sampling": {
        "peak_sampling": True,
        "gamma_peak_sampling": 1.2,
        "mix_uniform_ratio": 0.4,
    },

    "loss": {
        # ===== 点级 =====
        "use_smoothness": True,     # 保留你原来的“平滑正则”
        "smooth_w2": 0.4,
        "smooth_w1": 0.00,

        "use_peak_aux": True,       # A_aux（加权点级）
        "alpha_peak_weight": 5.0,
        "p_peak_weight": 2.0,
        "lambda_aux": 0.2,

        "use_mag_penalty": True,    # |S|>1 惩罚
        "mag_w": 0.05,

        # ===== 曲线段级（核心新增）：STFT 局部时频一致性（适合非周期震荡）=====
        # 作用对象：A_pred(λ段) vs A_true(λ段)
        "use_stft_curve": True,
        "stft_w": 0.15,              # 建议 0.05~0.3 试
        "stft_n_fft": 32,            # K=90 时 32/48/64 都可
        "stft_win_length": 32,
        "stft_hop_length": 8,
        "stft_center": False,        # False 更“段内一致”，避免 padding 影响
        "stft_log_mag": True,        # 用 log(1+|X|) 更稳
        "stft_norm_per_sample": True # 每条段归一化，避免能量尺度干扰形状
    },
    ...
    "early_stop": {
        ...
        "warmup": 50,
        "save_best": True,
        "best_name": "best_fourier2d_dwconv_stft_curve.pt",
    },

    "viz": {
        "clamp_mag_in_viz": True,
        "clamp_eps": 1e-6,
    },
}
...
class CurveSegmentSParamsDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        yS = np.stack([np.real(s11), np.imag(s11), np.real(s21), np.imag(s21)], axis=-1).astype(np.float32)
...
# ==========================================================
# 3) 模型：2D Fourier Encoder + DWConv(方案2) + λ FF + Head
# ==========================================================
class FourierOperator2d(nn.Module):
    ...
    def forward(self, x):
        ...
        weight = torch.complex(self.weight_real, self.weight_imag).to(dtype=torch.complex64)
...
class Fourier2DEncoder(nn.Module):
    """输出 (B,width)"""
    def __init__(self, modes=6, width=64, depth=4, use_groupnorm=False, gn_groups=8):
        super().__init__()
...
class Fourier2D_LambdaConditional_SParams(nn.Module):
    def __init__(self, modes=6, width=64, depth=4, lam_ff=8, head_hidden=256, use_groupnorm=False, gn_groups=8):
        super().__init__()
        self.encoder = Fourier2DEncoder(
            modes=modes, width=width, depth=depth,
            use_groupnorm=use_groupnorm, gn_groups=gn_groups
        )
...
def predict_full_S_and_A(model, pattern_11x11, lambda_vec, device, clamp_mag=True, clamp_eps=1e-6):
    ...
    out = model.head(torch.cat([z, le], dim=-1))
...
mc = CFG["model"]
model = Fourier2D_LambdaConditional_SParams(
    modes=mc["modes"],
    width=mc["width"],
    depth=mc["depth"],
    lam_ff=mc["lam_ff"],
    head_hidden=mc["head_hidden"],
    use_groupnorm=mc.get("use_groupnorm", False),
    gn_groups=mc.get("gn_groups", 8),
).to(device)
...
# ===== STFT window buffer（避免每步创建）=====
def _make_hann(win_length: int, device_: str):
    return torch.hann_window(win_length, periodic=True, dtype=torch.float32, device=device_)

stft_window = _make_hann(int(lc["stft_win_length"]), device)

def _stft_mag(x_1d: torch.Tensor):
    """
    x_1d: (B, K) float
    return: mag (B, F, T) float
    """
    n_fft = int(lc["stft_n_fft"])
    hop = int(lc["stft_hop_length"])
    win = int(lc["stft_win_length"])
    center = bool(lc["stft_center"])

    # 兜底：n_fft / win 不超过 K
    K = x_1d.size(-1)
    n_fft_eff = min(n_fft, K)
    win_eff = min(win, n_fft_eff)
    # 若 win_length 被截断，需要对应 window
    win_tensor = stft_window
    if win_tensor.numel() != win_eff or win_tensor.device != x_1d.device:
        win_tensor = _make_hann(win_eff, x_1d.device)
    ...
    if bool(lc["stft_log_mag"]):
        mag = torch.log1p(mag)

    if bool(lc["stft_norm_per_sample"]):
        # 每条样本归一化，强调“形状/分布”而非总能量
        denom = mag.mean(dim=(1, 2), keepdim=True).clamp_min(1e-6)
        mag = mag / denom
...
def compute_batch_loss(predS, yS, a_true):
    ...
    # ==========================================================
    # ✅ 新增：曲线段级 STFT loss（对非周期震荡友好）
    # 对比：A_pred(λ段) vs A_true(λ段)
    # ==========================================================
    if lc["use_stft_curve"] and K >= 8:
        A_pred = 1.0 - (ReS11 ** 2 + ImS11 ** 2) - (ReS21 ** 2 + ImS21 ** 2)  # (B,K)
        A_true = a_true.squeeze(-1)                                           # (B,K)

        # 可选：限制在合理范围（避免极端值影响 STFT）
        A_pred_c = A_pred.clamp(-0.5, 1.5)
        A_true_c = A_true.clamp(-0.5, 1.5)

        mag_p = _stft_mag(A_pred_c)
        mag_t = _stft_mag(A_true_c)

        stft_loss = ((mag_p - mag_t) ** 2).mean()
        loss = loss + float(lc["stft_w"]) * stft_loss
...
# ==========================================================
# 8) 训练 + TensorBoard + Early Stopping
# ==========================================================
...
plt.legend()
plt.tight_layout()
plt.savefig("loss_curve_dwconv_stft.png", dpi=200)
plt.show()
