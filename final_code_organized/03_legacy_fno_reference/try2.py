...
class PeakAwareTrainDataset(Dataset):
    def __init__(self, x_N11x11, lambda_vec, S11, S21, k_uniform=24, k_peak=24):
        ...
        R = np.abs(self.S11) ** 2
        T = np.abs(self.S21) ** 2
        self.A = np.clip(1.0 - R - T, 0.0, 1.0).astype(np.float32)
    ...
    def __getitem__(self, i):
        ...
        lam_n = torch.from_numpy(lam_n).unsqueeze(1)
...
def make_peak_weights(a_true):
    ...
    if a_true.shape[1] >= 3:
        curv = torch.abs(second_diff(a_true))
...
def peak_aware_loss(predS_raw, yS, a_true, lam_raw):
    """
    predS_raw:  (B, K, 4) 模型原始输出
    yS:         (B, K, 4)
    a_true:     (B, K, 1)
    lam_raw:    保留接口
    """
    A_true = a_true.squeeze(-1).clamp(0.0, 1.0)

    # 对原始输出越界做惩罚
    loss_phys = (power_excess(predS_raw) ** 2).mean()

    # 强制投影到被动域
    predS = project_to_passive(predS_raw)

    # 所有与吸收率相关的项都基于投影后的结果
    A_pred = s_to_A(predS).clamp(0.0, 1.0)
...
selected_11x11xN = extract_selected_11x11xN(patterns)
lambda_vec = extract_lambda_vec(sp)
...
def evaluate_full_spectrum(loader):
    ...
    for batch_id, (x, lam_n, yS, a_true, lam_raw) in enumerate(loader):
        ...
        loss, _ = peak_aware_loss(predS_raw, yS, a_true, lam_raw)
        ...
        predS = project_to_passive(predS_raw)
        A_pred = s_to_A(predS).clamp(0.0, 1.0).detach().cpu().numpy()
...
for epoch in range(1, EPOCHS + 1):
    ...
    for x, lam_n, yS, a_true, lam_raw in train_loader:
        ...
        loss, stats = peak_aware_loss(predS_raw, yS, a_true, lam_raw)
        loss.backward()
        optimizer.step()
